...
setwd("D:/BigDataAnalytics/Capitulo09/Exercicios")
getwd()

# Exercício 01 - Gere 1000 números de uma distribuição normal com média 3 e sd = .25 e grave no objeto chamado x.

?rnorm

x <- rnorm(1000, 3, .25)

# Exercício 02 - Crie o histograma dos dados gerados no item anterior e adicione uma camada com a curva da normal.

hist(x, prob=TRUE, ylim=c(0,1.80), breaks=20, main = "Histograma de x")
curve(dnorm(x, 3, 0.25), add=TRUE, col="red", lwd=1)

# Exercício 03 - Suponha que 80% dos adultos com alergias relatem alívio sintomático com uma medicação específica. 
...
# Exercício 04 - Suponha que os resultados dos testes de um vestibular se ajustem a uma distribuição normal. 
...
# Exercício 05 - Suponha que o tempo médio de check-out de um caixa de supermercado seja de três minutos. 
...
# Exercício 06 - Selecione dez números aleatórios entre um e três.
# Aplicamos a função de geração runif da distribuição uniforme para gerar dez números aleatórios entre um e três.

runif(10, min=1, max=3) 


# Exercício 07 - Se houver 12 carros atravessando uma ponte por minuto, em média, 
...
# Exercício 08 - Suponha que haja 12 questões de múltipla escolha em um questionário de inglês. 
