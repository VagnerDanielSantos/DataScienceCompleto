...
setwd("D:/BigDataAnalytics/Capitulo11/Classificacao")
getwd()
...
dados <- read.csv("D:/BigDataAnalytics/Capitulo11/Classificacao/dataset.csv", stringsAsFactors = FALSE)
str(dados)
View(dados)
...
# Cria uma função de normalização

normalizar <- function(x) {
  return ((x - min(x)) / (max(x) - min(x)))
}

# Testa a função de normalização - os resultados devem ser idênticos
...
# Carrega o pacote library

install.packages("class")
library(class)
?knn

# Cria dados de treino e dados de teste

dados_treino <- dados_norm[1:469, ]
dados_teste <- dados_norm[470:569, ]

# Cria os labels para os dados de treino e de teste
...
## Etapa 4: Avalia e Interpreta o Modelo

# Carrega o gmodels

library(gmodels)

# Cria uma tabela cruzada dos dados previstos x dados atuais
# Será usado amostra com 100 observações: length(dados_teste_labels)
...
# Confirma a transformação realizada com sucesso

summary(dados_z$area_mean)

# Cria novos datasets de treino e de teste
...
# Etapa 6: Constrói um Modelo com Algoritmo Support Vector Machine (SVM)
...
# Remove o índice dos datasets

trainset <- trainset[,-trainColNum]
testset <- testset[,-trainColNum]

# Obtém índice de coluna da variável target no conjunto de dados
