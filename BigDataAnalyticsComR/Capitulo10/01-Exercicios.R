...
setwd("D:/BigDataAnalytics/Capitulo10/Exercicios")
getwd()
...
# irá precisar do conhecimento adquirido em outros capítulos do curso 
# estudados até aqui para resolver esta lista de exercícios!


# Exercício 01 - Construa o dataset pop_data com os dados de voos das 
...
# Exercício 02  - Crie duas amostras de 1000 observações cada uma a partir do 
...
# Exercício 03 - Crie um dataset contendo os dados das 2 amostras criadas no item anterior. 

dataset <- rbind(amostra01,amostra02)
View(dataset)

# Exercício 04 - Calcule o intervalo de confiança (95%) da amostra1
...
erro_padrao_amostra01 = sd(amostra01$arr_delay) / sqrt(nrow(amostra01))

# Limites inferior e superior
# 1.96 é o valor de z score para 95% de confiança

lower = mean(amostra01$arr_delay) - 1.96 * erro_padrao_amostra01  
upper = mean(amostra01$arr_delay) + 1.96 * erro_padrao_amostra01

# Intervalo de confiança

ic_1 = c(lower,upper)
mean(amostra01$arr_delay)
ic_1
...
erro_padrao_amostra02 = sd(amostra02$arr_delay) / sqrt(nrow(amostra02))

# Limites inferior e superior
# 1.96 é o valor de z score para 95% de confiança

lower = mean(amostra02$arr_delay) - 1.96 * erro_padrao_amostra02
upper = mean(amostra02$arr_delay) + 1.96 * erro_padrao_amostra02

# Intervalo de confiança

ic_2 = c(lower,upper)
mean(amostra02$arr_delay)
ic_2

# Exercício 06 - Crie um plot Visualizando os intervalos de confiança criados nos itens anteriores
# Dica: Use o geom_point() e geom_errorbar() do pacote ggplot2

toPlot = summarise(group_by(dataset, sample_id), mean = mean(arr_delay))
...
delta <- sample_n(filter(pop_data, carrier == "DL", arr_delay > 0), 1000)
united <- sample_n(filter(pop_data, carrier == "UA", arr_delay > 0), 1000)

# Calcula erro padrão e média

se = sd(delta$arr_delay) / sqrt(nrow(delta))
mean(dl$arr_delay)

# Limites inferior e superior

lower = mean(delta$arr_delay) - 1.96 * se
upper = mean(delta$arr_delay) + 1.96 * se
ic_dl = c(lower,upper)
ic_dl

# Repete o processo para a outra companhia

se = sd(united$arr_delay) / sqrt(nrow(united))
mean(united$arr_delay)

lower = mean(united$arr_delay) - 1.96 * se
upper = mean(united$arr_delay) + 1.96 * se
ic_ua = c(lower,upper)
ic_ua
...
t.test(delta$arr_delay, united$arr_delay, alternative="greater")
