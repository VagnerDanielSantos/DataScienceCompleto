...
# Big Data na prática 04 - Customer Churn Analytics 
...
# Um setor no qual saber e prever as taxas de cancelamento e particularmente útil é o setor de telecomunicações, 
...
# 3. Como a permanência mínima é de 1 mês e a permanência máxima são de 72 meses, 
...
# Correlaçao entre variáveis numéricas
...
# Então, um deles será removido do modelo. NÃ³s removemos Total Charges.
...
plot05 <- ggplot(churn, aes(x=PhoneService)) + ggtitle("Phone Service") + xlab("Telefonia") +
  geom_bar(aes(y = 100*(..count..)/sum(..count..)), width = 0.5) + ylab("Percentual") + coord_flip() + theme_minimal()
plot06 <- ggplot(churn, aes(x=MultipleLines)) + ggtitle("Multiple Lines") + xlab("MÃºltiplas Linhas") + 
  geom_bar(aes(y = 100*(..count..)/sum(..count..)), width = 0.5) + ylab("Percentual") + coord_flip() + theme_minimal()
...
# Árvores de Decisão, elas são Contract, tenure_group e PaperlessBilling.
...
## Matriz de Confusão de Árvore de Decisão
